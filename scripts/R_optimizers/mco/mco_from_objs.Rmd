...
plot(x=round(k_n50_fit$par),y=seq(1,dim(k_n50_fit$par)[1],1), xlab="K value", ylab="Population individual", main="Parameter space at final iteration \n Optimization over \nObjective function N50", xlim=c(inparam$min,inparam$max))

plot(x=round(k_n50_fit$par),y=(-1* k_n50_fit$value[,1]), xlab="K value", ylab="Optimal N50", main="Convergent solution space \n Optimization over \nObjective function N50", xlim=c(inparam$min,inparam$max))

##1 input 2 responses
#plotdat=data.frame(k=k_n50_l50_fit$par[,1], n50=k_n50_l50_fit$value[,1], l50=k_n50_l50_fit$value[,2], model="k_n50_l50")

#plot(x=round(k_n50_l50_fit$par),y=seq(1,dim(k_n50_l50_fit$par)[1],1), xlab="K value", ylab="Population individual", main="Parameter space at final iteration \n Joint optimization over \nObjective functions N50 and L50", xlim=c(inparam$min, inparam$max))
